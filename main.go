...
参数：
	   ...
	   -m masscan.json
	       Masscan结果文件；导入 masscan -oJ 输出的 JSON 文件，自动读取 IP 和端口；
	       若同时指定 -tp 则忽略 JSON 中的端口，对 IP 去重后使用 -tp 指定的端口；(默认 空)
	   -o result.csv
...
`
	...
	flag.StringVar(&task.MasscanFile, "m", "", "Masscan结果文件")
